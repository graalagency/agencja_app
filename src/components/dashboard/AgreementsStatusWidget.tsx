...
export function AgreementsStatusWidget({ statuses, total, title, subtitle, size }: Props) {
  if (size === 'small') {
    return (
      <Card className="border-0 shadow-md bg-gradient-to-br from-sky-50 via-white to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 h-full flex flex-col">
          ...
          <table className="w-full h-full table-fixed">
            <tbody className="divide-y divide-border/50 text-xs">
              {statuses.map((item) => {
                ...
                return (
                  <tr key={item.code} className={`${SMALL_ROW_CLASS} hover:bg-sky-50 dark:hover:bg-sky-900/10 transition-colors`}>
        ...
        <table className="w-full h-full table-fixed">
          <tbody className="divide-y divide-border/30 text-sm">
            {statuses.map((item) => {
              ...
              return (
                <tr key={item.code} className={`${LARGE_ROW_CLASS} hover:bg-sky-50 dark:hover:bg-sky-900/10 transition-colors`}>
                  ...
                  <td className="text-right px-6 align-middle text-xs text-muted-foreground w-28">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <div className="w-12 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div className={`h-1.5 rounded-full bg-sky-500`} style={{ width: `${percent}%` }} />
                      </div>
                    </div>
                  </td>
                </tr>
