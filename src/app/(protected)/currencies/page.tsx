...
import { DictCurrencyCreateSchema, DictCurrencyUpdateSchema } from '../../../validation/currency-dictionary'
...
export default function CurrenciesPage() {
  const t = useTranslations('currencyDictionary')
  const tCommon = useTranslations('common')

  const [items, setItems] = useState<DictCurrency[]>([])
  ...
  const [editingItem, setEditingItem] = useState<DictCurrency | null>(null)
  ...
  const load = async (page = meta.page) => {
    ...
    const res = await fetch(`/api/currencies?${qs.toString()}`)
    if (!res.ok) {
      setItems([])
  ...
  const addItem = async () => {
    ...
    const parsed = DictCurrencyCreateSchema.safeParse(payload)
    if (!parsed.success) {
  ...
  const updateItem = async () => {
    ...
    const parsed = DictCurrencyUpdateSchema.safeParse(payload)
    if (!parsed.success) {
      setFormErrors(parsed.error.errors.map((e) => e.message))
      return
    }
    ...
    setFormErrors([])
    const res = await fetch(`/api/currencies/${editingItem.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(parsed.data),
    })
  ...
  const openEdit = (item: DictCurrency) => {
    setEditingItem(item)
    setForm({
