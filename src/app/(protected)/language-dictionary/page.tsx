...
import { DictLanguageCreateSchema, DictLanguageUpdateSchema } from '../../../validation/language-dictionary'
...
export default function LanguageDictionaryPage() {
  const t = useTranslations('languageDictionary')
  const tCommon = useTranslations('common')

  const [items, setItems] = useState<DictLanguage[]>([])
  const [meta, setMeta] = useState<Meta>({ page: 1, pageSize: 10, total: 0, pages: 1 })
  const [loading, setLoading] = useState(true)

  const [search, setSearch] = useState('')
  const [sortBy, setSortBy] = useState<'id' | 'description' | 'code' | 'createdAt'>('id')
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc')
  const [pageSize, setPageSize] = useState(10)

  const [showAddModal, setShowAddModal] = useState(false)
  const [editingItem, setEditingItem] = useState<DictLanguage | null>(null)
  ...
  const load = async (page = meta.page) => {
    ...
    const res = await fetch(`/api/language-dictionary?${qs.toString()}`)
    if (!res.ok) {
      setItems([])
  ...
  const addItem = async () => {
    ...
    const parsed = DictLanguageCreateSchema.safeParse(payload)
    if (!parsed.success) {
    ...
    setFormErrors([])
    const res = await fetch('/api/language-dictionary', {
      method: 'POST',
  ...
  const updateItem = async () => {
    ...
    const parsed = DictLanguageUpdateSchema.safeParse(payload)
    if (!parsed.success) {
      setFormErrors(parsed.error.errors.map((e) => e.message))
      return
    }
    ...
    setFormErrors([])
    const res = await fetch(`/api/language-dictionary/${editingItem.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(parsed.data),
    })
  ...
  const openEdit = (item: DictLanguage) => {
    setEditingItem(item)
    setForm({
  ...
  const removeItem = async (id: number) => {
    ...
    await fetch(`/api/language-dictionary/${id}`, { method: 'DELETE' })
    await load(meta.page)
  }
    ...
    <div className="space-y-6">
      <Card className="p-6">
        ...
        <div className="max-w-md">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`${t('description')}/${t('abbreviation')}`} />
        </div>
      </Card>
            ...
            <Table>
              <thead>
                <tr>
                  ...
                  <Th onClick={() => toggleSort('code')} active={sortBy === 'code'} order={sortOrder}>{t('abbreviation')}</Th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 overflow-y-auto">
          <div className="bg-white dark:bg-gray-800 p-6 rounded shadow w-full max-w-xl my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <label className="label">{t('abbreviation')} *</label>
